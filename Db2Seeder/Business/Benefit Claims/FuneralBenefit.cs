    ...
    public class FuneralBenefit
    {
        public static async Task<List<SupportRequest>> GetClaimsCompleted()
        {
            try
            {
                ...
                return RequestList = await ApiRequest.GetSupportRequestTypeByState(8, 223);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static async Task<Document_FuneralBenefit> ClaimDetail(SupportRequest Request)
        {
            try
            {
                ...
                if (guid.message != Guid.Empty)
                {
                    ...
                    Document_FuneralBenefit.CreatedOn = requestHistory.OrderBy(x => x.dateModified).Where(x => x.description.Contains("Pending Processing")).First().dateModified.ToLocalTime();
