    ...
    public class CovidBenefit
    {
        ...
        public static async Task<List<SupportRequest>> GetClaimsCompleted()
        {
            try
            {
                ...
                //actualizado yaaa
                return RequestList = await ApiRequest.GetSupportRequestTypeByState(14, 228);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static async Task<Document_Covid19> ClaimDetail(SupportRequest Request)
        {
            ...
            {                
                ...
                if (guid.message != Guid.Empty)
                {
                    ...
                    Document_Covid19.CreatedOn = requestHistory.OrderBy(x => x.dateModified).Where(x => x.description.Contains("Employer Approved")).Last().dateModified.ToLocalTime();
