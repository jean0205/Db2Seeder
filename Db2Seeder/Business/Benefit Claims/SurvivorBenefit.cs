    ...
    public class SurvivorBenefit
    {
        ...
        public static async Task<List<SupportRequest>> GetClaimsCompleted()
        {
            try
            {
                ...
                return RequestList = await ApiRequest.GetSupportRequestTypeByState(11, 224);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static async Task<Document_SurvivorBenefit> ClaimDetail(SupportRequest Request)
        {
            try
            {
                ...
                if (guid.message != Guid.Empty)
                {
                    ...
                    Document_SurvivorBenefit.CreatedOn = requestHistory.OrderBy(x => x.dateModified).Where(x => x.description.Contains("Pending Processing")).First().dateModified.ToLocalTime();
                ...
                if (attachmentsGuid.Any())
                {
                    ...
                    if (document_MetaDataList.Any())
                    {
                        ...
                        foreach (var item in document_MetaDataList)
                        {
                            ...
                            documents.ClaimNumber = Document_SurvivorBenefit.ClaimNumber;
