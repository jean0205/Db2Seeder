        ...
        public static async Task<Document_SurvivorBenefit> ClaimDetail(SupportRequest Request)
        {
            try
            {
                ...
                if (guid.message != Guid.Empty)
                {
                    ...
                    Document_SurvivorBenefit.CreatedOn = requestHistory.OrderBy(x => x.dateModified).Where(x => x.description.Contains("Pending Processing.")).Last().dateModified.ToLocalTime();
